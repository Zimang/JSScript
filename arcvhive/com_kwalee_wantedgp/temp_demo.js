...
var mode=1;
let proj="com_kwalee_wantedgp"
let projP="com.kwalee.wantedgp.autojs"
let projR="com.kwalee.wantedgp"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    if(tests[6].existApply(no_action)){
        isGuid=false
        while(true){
            press(100,1750,2000) //左
            if(tests[7].existApply(ddclickCenter)){
                break
            }
            press(900,1750,2000) //右   
...
function main(){
    ...
    app.launch(projR)
    // app.startActivity({ 
    //     packageName: "com.kwalee.wantedgp",
