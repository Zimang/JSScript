...
var mode = 1;
let proj = "com_Trailmix_LoveAndPiesMerge"
let projP = "com.Trailmix.LoveAndPiesMerge.autojs"
let projR = "com.Trailmix.LoveAndPiesMerge"
function getPath(p) {
    switch (mode) {
        case 5:
...
//竖版
let TAP = {
    x: device.width / 2, y: device.height / 2,
    cl: function () {
        click(this.x, this.y)
    },
    clp: function (a, b) {
...
function guid() {
    ...
    if (tests[1].existApply(ddclickCenter)) {
        ...
        while (!tests[3].existApply((x, y, w, h) => {
            click(350, 210)
        })) {
            gs([ [400, 1070]
                , [530, 1070]
        ...
        while(!tests[10].existApply((x,w,z,d)=>{
            click(350, 210)
        })){
            
            cclick([[680,1200],[680,1200],[680,1200],[680,1200],[680,1200]],500)  
            gs([ [400, 1070]
                , [530, 1070]
                , [ 400,1200 ] 
                , [  250,1200  ] 
        ...
        tests[8].waitMe((x,w,z,d)=>{
            click(140,230)
        },200,1000)
        tests[5].waitMe((x,w,z,d)=>{
            click(530,1355)
        },200,1000)

        
        while (!tests[6].existApply(ddclickCenter)) {
            cclick([[910,94],[910,94],[910,94]],500) 
        }
        tests[9].waitMe(ddclickCenter,200,1000)
...
// 定义三个无副作用的逻辑函数
function logic1() {
    if(!isGuid&& tests[11].existApply(ddclickCenter)){
        sleep(1500) 
        while(!tests[11].existApply(no_action)){
            click(920,1800) 
            sleep(1000)
        }
        sleep(500)
...
function logic2() {
    if(!isGuid&&tests[11].existApply(no_action)){
        click(110,1800)
        sleep(1500)
