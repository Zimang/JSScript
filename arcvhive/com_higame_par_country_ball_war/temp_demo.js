...
var mode=1;
let proj="com_higame_par_country_ball_war"
let projP="com.higame.par.country.ball.war.autojs"
let projR="com.higame.par.country.ball.war"
function getPath(p){
    switch(mode){
        case 5:
...
let nextBT=new zutils.CachedBT(
    "cut_4_1.png",getPath(""),"NEXT"
) 
let tap=new zutils.CachedBT(
    "cut_1_1.png",getPath(""),"tap"
) 
let noThanks=new zutils.CachedBT(
    "cut_5_1.png",getPath(""),"no thanks"
) 
function shift(){ 
...
//对于游戏我们只需要找到一个死循环即可
function singleLoop(){
    if(tap.existApply(ddclickCenter)){
    
        sleep(20000)
        while(!nextBT.existApply(ddclickCenter)){
            // if(c++==count){
            //     break
            // }
            img = captureScreen()
            ...
            sleep(2000)
            pointM = findMultiColors(source, "#e52e26", [
                [30, 30, "#e52e26"],  // 主颜色点 (x=0,y=0) 的右侧 10px, 下方 10px 必须是绿色
                [40, 40, "#e52e26"],    // 主颜色点的右侧 20px, 下方 20px 必须是蓝色
                [50, 50, "#e52e26"],    // 主颜色点的右侧 20px, 下方 20px 必须是蓝色
                [60, 60, "#e52e26"],    // 主颜色点的右侧 20px, 下方 20px 必须是蓝色
            ],{region: [0, TAP.y*0.2, device.width, device.height-TAP.y*0.2],
                threshold: 40
            });  
            attack(pointM)
            source.recycle()
...
function attack(pointM,ran,len,delay){
    if(pointM==undefined){
        return
    }

    if(len==undefined){
        len=getRandomInt(100,200)
    }
    if(ran==undefined){
        ran=getRandomInt(0,5)
    }
    if(delay==undefined){
        delay=500
    }
    ...
    if(ran==1){
        gesture(500,[pointM.x,pointM.y],[pointM.x+len,pointM.y+len])
    }else if(ran==2){
        gesture(500,[pointM.x,pointM.y],[pointM.x-len,pointM.y+len])
    }else if(ran==3){
        gesture(500,[pointM.x,pointM.y],[pointM.x+len,pointM.y-len])
