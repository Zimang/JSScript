...
var mode=1;
let proj="com_trustedapp_qrcodebarcode"
let projP="com.trustedapp.qrcodebarcode.autojs"
let projR="com.trustedapp.qrcodebarcode"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    while(
        tests[1].existApply(no_action)
        // zutils.afollow([
        //     ["i","com.trustedapp.qrcodebarcode:id/btnStartScan",0,500,no_action],
        //     /**Start with sample QR 
        //      * Good QR example
        //     */
        // ])
    ){
        click(980+getRandomInt(-20,20),309+getRandomInt(-20,20))
        sleep(500)
    } 
    if(zutils.afollow([
        ["t","Select a language",0,0,no_action],
    ])){
        zutils.afollow([
            ["t","English",0,500],
        ])
        ...
        zutils.afollow([
            ["i","com.trustedapp.qrcodebarcode:id/buttonLanguageNext",0,500],
        ])
        right2Left()
        right2Left()
        right2Left()
        ...
        while(zutils.afollow([
            ["t",'Choose "Continue" to claim your exclusive gift.',0,500,no_action],
        ])){ 
            zutils.afollow([
                ["t","Continue",0,3000],
            ])
...
function logic1() {  
    ...
    if(zutils.afollow([
        ["t","Create QR Code",0,500,no_action],
    ])){
        if(zutils.afollow([
            ["t","Share your contact info",0,500],
        ])){
            while(!zutils.afollow([
                ["t","Create QR",0,500,goBack],
            ])){ }
        }
    }
}
function goBack(x,y,w,h){
    sleep(500)
    click(1024,88)
    sleep(500)
}
...
function logic3() { 
    ...
    zutils.afollow([
        ["i","com.trustedapp.qrcodebarcode:id/switch_vibrate",0,500],
    ])
    zutils.afollow([
        ["i","com.trustedapp.qrcodebarcode:id/switch_sound",0,500],
    ])  
...
function singleTest(){ 
    ...
    // logic3() 
    // singleLoop()    
    while(tests[1].existApply(no_action)){
        click(980+getRandomInt(-20,20),309+getRandomInt(-20,20))
        sleep(500)
    } 
}
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.trustedapp.qrcodebarcode",
