...
var mode=1;
let proj="net_wyvernware_whosthealien"
let projP="net.wyvernware.whosthealien.autojs"
let projR="net.wyvernware.whosthealien"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    if(tests[4].existApply(no_action)){
        TAP.cl() //start game
        sleep(1000)
        while(!tests[3].existApply(clickNoThanks)){
            // 打死小女孩
            clog("点击1")
            click(223,1545)
            sleep(500)
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "net.wyvernware.whosthealien",
