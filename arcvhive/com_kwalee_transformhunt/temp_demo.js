...
var mode=1;
let proj="com_kwalee_transformhunt"
let projP="com.kwalee.transformhunt.autojs"
let projR="com.kwalee.transformhunt"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    if(!flag&&shopLight.existApply(no_action)){
        TAP.cl()
        while(!lo3.existApply(no_action)){

        }
        gesture(5000,[TAP.x,TAP.y],[TAP.x+70,TAP.y-400])
        while(!bar.existApply(ddclickCenter)){
            gesture(5000,[TAP.x,TAP.y],[TAP.x,TAP.y-400])
        }
        gesture(5000,[TAP.x,TAP.y],[TAP.x,TAP.y-400])
 
        while(!hide.existApply(ddclickCenter)){

        }
        while(!nthanks.existApply(ddclickCenter)){
        }
        sleep(5000)
        m220.existApply(ddclickCenter)
        while(!shop.existApply(ddclickCenter)){
        }
        sleep(5000)
        done.existApply(ddclickCenter)

        while(!shopLight.existApply(no_action)){
        }
        TAP.cl()
        while(!box.existApply(no_action)){
        }
        der.drawCircle(10)
        sleep(1000)
        hide.existApply(ddclickCenter)
        while(!m230.existApply(getM)){

        }
        sleep(5000)
        m230.existApply(getM)
        flag=true
    }
...
function singleLoop(){
    ...
    if(flag&&shopLight.existApply(no_action)){ 
        ...
        while(!box2.existApply(no_action)){

        }
        
        gesture(5000,[TAP.x,TAP.y],[TAP.x,TAP.y-400]) 
        sleep(5000)
        while(!box3.existApply(no_action)){

        }
        sleep(500)
        box3.existApply(getM)
...
let shop=new zutils.CachedBT(
    "cut_1_1.png",getPath(""),"buttom 1"
) 
let nthanks=new zutils.CachedBT(
    "cut_4_1.png",getPath(""),"buttom 1"
) 
let m220=new zutils.CachedBT(
    "cut_5_1.png",getPath(""),"buttom 1"
) 
let m230=new zutils.CachedBT(
    "cut_10_1.png",getPath(""),"buttom 1"
) 

let hide=new zutils.CachedBT(
    "cut_3_1.png",getPath(""),"buttom 1"
) 
let done=new zutils.CachedBT(
    "cut_6_1.png",getPath(""),"buttom 1"
)  
let shopLight=new zutils.CachedBT(
    "cut_7_1.png",getPath(""),"buttom 1"
) 
let lo=new zutils.CachedBT(
    "cut_8_1.png",getPath(""),"buttom 1"
)  
let lo3=new zutils.CachedBT(
    "cut_8_3.png",getPath(""),"buttom 1"
)  
let box=new zutils.CachedBT(
    "cut_9_1.png",getPath(""),"buttom 1"
) 
let box2=new zutils.CachedBT(
    "cut_11_2.png",getPath(""),"buttom 1"
) 
let box3=new zutils.CachedBT(
    "cut_12_1.png",getPath(""),"buttom 1"
)  
let bar=new zutils.CachedBT(
...
function singleTest(){
    ...
    //     ["t","Next",1000,1000],  
    // ]) 
    box2.existApply(no_action)
    box3.existApply(no_action)
    // shopLight.existApply(no_action)
    
    // gesture(5000,[TAP.x,TAP.y],[TAP.x,TAP.y-400]) 

    // if(shopLight.existApply(no_action)){
    //     TAP.cl()   
    ...
    // hide.existApply(ddclickCenter)
    // shop.existApply(ddclickCenter) //only once
    // done.existApply(ddclickCenter)
    // TAP.cl() 
