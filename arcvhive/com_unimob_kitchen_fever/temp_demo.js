...
var mode=1;
let proj="com_unimob_kitchen_fever"
let projP="com.unimob.kitchen.fever.autojs"
let projR="com.unimob.kitchen.fever"
function getPath(p){
    switch(mode){
        case 5:
...
function ddEndTop(centerX,centerY,w,h){
    click(TAP.x*1.9,TAP.y*0.1)
    sleep(100) 
}
 
function ddCus(centerX,centerY,w,h){
    click(100,520)
    sleep(100) 
}
function ddPet(centerX,centerY,w,h){
    click(100,820)
    sleep(100) 
}
...
function left2Right(){ 
    // swipe(TAP.x,TAP.y,TAP.x,TAP.y-400,200) 
    gesture(1000,[0,TAP.y],[TAP.x,TAP.y])  
    sleep(500)
    return true
    // press(TAP.x*0.75,TAP.y*1.5,1000)  
}
...
function logic1() {  
    if(zutils.afollow([
        ["c",tests[5],0,0,no_action], 
    ])){
        ddCus()
        zutils.afollow([
            ["c",tests[2],0,1000,ddclickCenter], 
        ])
    }
...
function logic2() { 
    if(zutils.afollow([
        ...
        ddPet()
        zutils.afollow([
            ["c",tests[3],0,1000,ddEndTop], //宠物页面
        ])
    }
...
function logic3() { 
    if(zutils.afollow([
        ...
        left2Right()
        der.drawCircle(1,500) 
        der.drawCircleBackwards(1,800) 
    } 
...
let der=zutils.createCircleDrawer(TAP.x,TAP.y,20,200) 
function adapt(){ 
...
function singleTest(){
    ...
    // } 
    // zutils.afollow([
    //     // ["c",tests[3],0,0,no_action], 
    //     // ["c",tests[1],0,0,ddclickCenter],   
    //     // ["c",tests[13],0,0,no_action], 
    //     ["c",tests[18],0,0,no_action], 
    //     // ["c",tests[11],0,0,ddclickCenter], //自定义界面
    //     // ["c",tests[14],0,0,no_action], 
    //     // ["c",tests[16],0,0,no_action], 
    //     // ["c",tests[17],0,0,ddEndTop], //宠物页面
    // ]) 
