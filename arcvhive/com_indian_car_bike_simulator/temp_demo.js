...
var mode=1;
let proj="com_indian_car_bike_simulator"
let projP="com.indian.car.bike.simulator.autojs"
let projR="com.indian.car.bike.simulator"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    if(tests[6].existApply(ddclickCenter)){
        sleep(500)
        tests[5].waitMe(ddclickCenter,500,500)
        sleep(500)
    }
...
function singleLoop(){
    var coun=0
    while(tests[3].existApply(no_action)){
        tests[3].existApply((x,y,w,h)=>{ 
            der.drawCircle(1,getRandomInt(5000,6000))
        })
        tests[3].existApply((x,y,w,h)=>{ 
            der2.drawCircle(1,getRandomInt(5000,6000))
        }) 
        coun++;
        if(coun>5){
...
let der=zutils.createCircleDrawer(TAP.x+300,TAP.y,200,800) 
let der2=zutils.createCircleDrawer(TAP.x-500,TAP.y+100,200,800) 
