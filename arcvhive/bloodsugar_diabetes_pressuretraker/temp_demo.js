...
var mode=1;
let proj="bloodsugar_diabetes_pressuretraker"
let projpg="bloodsugar.diabetes.pressuretraker.autojs"
function getPath(p){
    switch(mode){
        case 5:
...
function shift(){ 
    swipe(TAP.x,TAP.y,TAP.x,TAP.y-200,200) 
    sleep(500)
    swipe(TAP.x,TAP.y,TAP.x,TAP.y+200,200) 
}
function shift2Left(){ 
    swipe(TAP.x,TAP.y,0,TAP.y,500)  
}

function sclcik(x,y,w,h){
    click(x+(w/2),y+(h/2))
    click(x+(w/2),y+(h/2))
}
var isGuid=true
...
function main(){
    ...
    // // warmup play
    console.log("start")
    app.launch("bloodsugar.diabetes.pressuretraker")
    sleep(5000)
