...
var mode=1;
let proj="com_flatgames_patrolofficer"
let projP="com.flatgames.patrolofficer.autojs"
let projR="com.flatgames.patrolofficer"
function getPath(p){
    switch(mode){
        case 5:
...
//对于游戏我们只需要找到一个死循环即可
function singleLoop(){
    tests[15].existApply(ddclickCenter)
    tests[9].existApply(ddclickCenter)
    if(tests[12].existApply(no_action)){
        tests[10].existApply(ddclickCenter)
    }
    if(tests[13].existApply(no_action)){
        tests[14].existApply(ddclickCenter)
    }
    tests[11].existApply(ddclickCenter)
}
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.flatgames.patrolofficer",
