...
var mode=1;
let proj="com_firemax_skintool_clash_emotes_royalepass"
let projP="com.firemax.skintool.clash.emotes.royalepass.autojs"
let projR="com.firemax.skintool.clash.emotes.royalepass"
function getPath(p){
    switch(mode){
        case 5:
...
let iv_1="com.firemax.skintool.clash.emotes.royalepass:id/imgClose"
let iv_2="com.firemax.skintool.clash.emotes.royalepass:id/ivSetting"
let iv_3="com.firemax.skintool.clash.emotes.royalepass:id/ivBack"
let iv_4="com.firemax.skintool.clash.emotes.royalepass:id/imgBack"
let TAP={x:device.width/2,y: device.height/ 2}

function shift(){ 
    // swipe(TAP.x,TAP.y,TAP.x,TAP.y-400,200) 
    gesture(1000,[TAP.x,TAP.y-100],[TAP.x,TAP.y-500])
    sleep(200)     
    gesture(1000,[TAP.x,TAP.y-500],[TAP.x,TAP.y-100]) 
    sleep(200)  
}
function shiftL(){ 
    // swipe(TAP.x,TAP.y,TAP.x,TAP.y-400,200) 
    gesture(800,[TAP.x+200,TAP.y],[0,TAP.y])   
    sleep(200)
}
var isGuid=true
function guid(){
    if(zutils.tfollow([
        ...
        if(zutils.tfollow([ ["Get Started",500]])){
            sleep(1000)
            zutils.idfollow([
                [iv_1,500],
            ])
