...
var mode=1;
let proj="com_blitzkrieg_assault"
let projP="com.blitzkrieg.assault.autojs"
let projR="com.blitzkrieg.assault"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    // ])  
    tests[11].existApply(t2play)
    if(tests[1].existApply(no_action)){
        tests[4].existApply(ddclickCenter) 
    } 
    tests[1].existApply(applySkillPoint)
    sleep(2000)
    tests[3].existApply(ddclickCenter)
    tests[10].existApply(ddclickCenter)
    zutils.afollow([
        ["t","This device isn’t Play Protect certified",0,0,backForGoogleProtect]
    ])
    zutils.afollow([
    ...
    tests[6].existApply(ddclickCenter)
    if(tests[2].existApply(no_action)){
        TAP.cl()
    }
    tests[12].existApply(ddclickCenter)
    tests[8].existApply(ddclickCenter)
    sleep(500)
    tests[9].existApply(dddclickCenter) 
}
function shift(){ 
    // swipe(TAP.x,TAP.y,TAP.x,TAP.y-400,200) 
...
function t2play(x,y,w,h){
    press(415,1500,2000)
    // gesture(2000,[415,1500],[435,1400])
    // gesture(2000,[435,1400],[415,1500])
}
...
function applySkillPoint(x,y,w,h){
    click(280,950)
    sleep(1000)
    click(560,950)
    sleep(200) 
...
function singleTest(){ 
    // guid()
    if(tests[11].existApply(t2play)){
        while(!tests[5].existApply(ddclickCenter)){
            while(!tests[4].existApply(ddclickCenter)
            &&!tests[6].existApply(no_action)){
                t2play() 
                tests[5].existApply(no_action)
            }
            while(!tests[1].existApply(applySkillPoint)
            &&!tests[6].existApply(no_action)){
                sleep(4000) 
                tests[5].existApply(no_action)
            }
            sleep(500)
            while(!tests[3].existApply(ddclickCenter)
            &&!tests[6].existApply(no_action)){
                sleep(4000) 
                tests[5].existApply(no_action)
            } 
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.blitzkrieg.assault",
