...
var mode=1;
let proj="com_bbg_one_line_puzzle_games"
let projP="com.bbg.one.line.puzzle.games.autojs"
let projR="com.bbg.one.line.puzzle.games"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    grant() 
    gotit.existApply(ddclickCenter,false)
}
function shift(){ 
    // swipe(TAP.x,TAP.y,TAP.x,TAP.y-400,200) 
...
function logic1() { 
    ...
    if(next.existApply(no_action)||nextx.existApply(no_action)){ 
        if(pointM){
            clog(pointM)
            var derr=zutils.createCircleDrawer(pointM.x,pointM.y+TAP.x*0.6,TAP.x*0.85,100)
            derr.drawCircle(20,4000)
        }else{
            clog("无色找到")
            img = captureScreen()
...
let next=new zutils.CachedBT(
    "cut_2_1.png",getPath(""),"buttom 1"
) 
let nextx=new zutils.CachedBT(
    "cut_x_1.png",getPath(""),"buttom 1"
) 
let clane=new zutils.CachedBT(
    "cut_2_2.png",getPath(""),"buttom 1"
)  
let ads=new zutils.CachedBT(
    "cut_2_4.png",getPath(""),"buttom 1"
) 
let replay=new zutils.CachedBT(
    "cut_3_1.png",getPath(""),"buttom 1"
) 
let gotit=new zutils.CachedBT(
    "cut_1_1.png",getPath(""),"buttom 1"
) 
let info=new zutils.CachedBT(
    "cut_2.png",getPath(""),"buttom 1"
) 
...
function singleTest(){
    ...
    // next.existApply(no_action)
    // clane.existApply(no_action)
    // settings.existApply(no_action)
    // ads.existApply(no_action)
    // replay.existApply(no_action)
    gotit.existApply(it)
    info.existApply(no_action)
