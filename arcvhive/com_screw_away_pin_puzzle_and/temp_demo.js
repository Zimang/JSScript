...
var mode=1;
let proj="com_screw_away_pin_puzzle_and"
let projP="com.screw.away.pin.puzzle.and.autojs"
let projR="com.screw.away.pin.puzzle.and"
function getPath(p){
    switch(mode){
        case 5:
...
function twoFingerMove(){
    gestures(
        [0, 500, [500, 800], [400, 700]], // 第一根手指从(500,800)到(400,700)
        [0, 500, [500, 800], [600, 900]]  // 第二根手指从(500,800)到(600,900)
    ); 
}
function guid(){
    grant()
...
function singleLoop(){
    ...
    if(!isGuid){
        while(tests[5].existApply(no_action)){
            click(getRandomInt(200,820),getRandomInt(730,1450))
        }
        sleep(200)
        if(tests[6].existApply(ddclickCenter)){
            tests[7].waitMe(ddclickCenter,500,500)
        }
    }
}
