...
var mode=1;
let proj="com_beat_fight_fnf_mobile_original_friday_night_funkin_game_android"
let projP="com.beat.fight.fnf.mobile.original.friday.night.funkin.game.android.autojs"
let projR="com.beat.fight.fnf.mobile.original.friday.night.funkin.game.android"
function getPath(p){
    switch(mode){
        case 5:
...
function catchRightPoint(x,y,w,h){  
...
function guid(){
    ...
    if(zutils.follow([ 
       [ "cut_13_2.png",500],
        ...
        while(!zutils.clickTargetPicCentralFromPath(getPath("cut_17_3.png"),500)&&
            !zutils.clickTargetPicCentralFromPath(getPath("cut_14_1.png"),0)){ 
...
function shift(){ 
    ...
    if(rp.x&&dp.x&&up.x&&lp.x){
        gesture(200,[lp.x,lp.y],[rp.x,lp.y]) 
        gesture(200,[lp.x,lp.y],[rp.x,lp.y]) 
    } 
}
...
function catchBT(){
    if(!(rp.x&&dp.x&&up.x&&lp.x)){
        zutils.follow([
            [ "cut_13_4.png",500], 
            ],getPath(""),catchRightPoint)
        zutils.follow([
            [ "cut_13_2.png",500], 
            ],getPath(""),catchDownPoint)
