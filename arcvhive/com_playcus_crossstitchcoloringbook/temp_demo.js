...
var mode=1;
let proj="com_playcus_crossstitchcoloringbook"
let projP="com.playcus.crossstitchcoloringbook.autojs"
let projR="com.playcus.crossstitchcoloringbook"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    tests[7].existApply(ddclickCenter) 
    
    if(tests[6].existApply(no_action)){  //退出游戏的逻辑
        ddGreen()
    }
    tests[8].existApply(ddclickCenter)
    if(tests[9].existApply(no_action)){  //退出游戏的逻辑
        TAP.clp(0.2,0.1)
        // sleep(1000)
        // if(tests[2].existApply(no_action)){ 
        //     tests[2].waitMe((x,y,w,h)=>{
        //         TAP.clp(1.8,0.1) 
        //     },300,1000)
        // }
        // sleep(1000) 
        ...
        if(!isB){
            ...
            sleep(3000)
            TAP.clp(1.8,0.1) 
            // tests[10].waitMe((x,y,w,h)=>{
            // },300,20)  
        }
  
        tests[4].waitMe(ddclickCenter,500,10)
    }
  
}
function logic2() { 

    if(tests[5].existApply(no_action)){  //退出游戏的逻辑
        TAP.clp(1.85,0.1)
        sleep(1000)
        TAP.clp(0.5,0.1) 
    }
...
function logic3() { 
    if(tests[5].existApply(no_action)){   
        ...
        if(!tests[5].existApply(no_action)){
            ...
            if(tests[9].waitMe(no_action,300,10)){  //退出游戏的逻辑
                TAP.clp(0.2,0.1) 
