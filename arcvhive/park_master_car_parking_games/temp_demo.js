...
var mode=1;
let proj="park_master_car_parking_games"
let projP="park.master.car.parking.games.autojs"
let projR="park.master.car.parking.games"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    if(isGuid){
        ...
        if(tests[6].existApply(no_action)){
            gesture(1000,[717,792],[767,535])
            sleep(1000)
            gesture(1000,[366,781],[717,869])
            sleep(1000)
            gesture(1000,[381,1023],[915,1102])
            sleep(1000)
        } 
    }
    if(tests[7].existApply(ddclickCenter)){
        isGuid=false
    }
    if(!isRewarded){
        if(tests[5].existApply(no_action)){
            click(285,650)
            sleep(1000)
            if(!tests[5].existApply(no_action)){
                isRewarded=true
...
function singleLoop(){
    ...
    if(tests[3].existApply(no_action)){
        click(537,1302)
        sleep(2000)
    }
...
let c2p=zutils.create2PointGenerator(600,1000,100,337,404)
function adapt(){ 
...
function sscrp(){
    ...
    for(i;i<times;i++){
        ...
        var dddd= c2p()
        if(i%2==1){
            gesture(getRandomInt(300,500),[dddd[0].x,dddd[0].y],[dddd[1].x,dddd[1].y]) 
        }else{
            gesture(getRandomInt(300,500),[dddd[1].x,dddd[1].y],[dddd[0].x,dddd[0].y])
        } 
    }
}
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "park.master.car.parking.games",
