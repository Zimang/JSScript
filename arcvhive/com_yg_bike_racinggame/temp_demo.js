...
var mode=1;
let proj="com_yg_bike_racinggame"
let projP="com.yg.bike.racinggame.autojs"
let projR="com.yg.bike.racinggame"
function getPath(p){
    switch(mode){
        case 5:
...
function clickLevelOne(x,y,w,h){ 
    click(425,657) 
}
function guid(){
    grant()
    ...
    while(tests[1].existApply(clickLevelOne)){ 
        sleep(2000)
        if(tests[3].existApply(no_action)){
            press(1654,725,20000)  
        }
    }
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.yg.bike.racinggame",
