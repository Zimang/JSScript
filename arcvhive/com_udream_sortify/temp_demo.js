...
var mode=1;
let proj="com_udream_sortify"
let projP="com.udream.sortify.autojs"
let projR="com.udream.sortify"
function getPath(p){
    switch(mode){
        case 5:
...
function singleLoop(){
    while(tests[5].existApply(no_action)){
        gesture(1000,[652,959],[281,771])
        sleep(500)
        gesture(1000,[545,975],[773,787])

        tests[1].waitMe(ddclickCenter)
        // while(!tests[1].existApply(ddclickCenter)){
        //     sleep(500)
        // }
        sleep(1000)
        var count=0
        while(tests[1].existApply(ddclickCenter)){
            ...
            count++
            if(count==4){ 
                tests[2].waitMe(ddclickCenter)
                tests[3].waitMe(ddclickCenter)
        ...
        if(!tests[3].waitMeBy(()=>{
            return tests[1].existApply(ddclickCenter)
        },ddclickCenter)){
            tests[2].waitMe(ddclickCenter)
            tests[3].waitMe(ddclickCenter)
            tests[4].waitMe(ddclickCenter)
...
/**
 * 11:01:33.684/D: 缩放比： [ 1, 1 ] 可疑目标数： 1
11:01:33.686/D: { x: 179.5,
  y: 1469.5,
  w: 141,
  h: 169,
  tag: 'button 1',
  mauto: false,
  img: '1.png',
  prefix: '/storage/emulated/0/脚本/com_udream_sortify/com_udream_sortify/assests/',
  delay: 500,
  actionDelay: 100 }
 */
