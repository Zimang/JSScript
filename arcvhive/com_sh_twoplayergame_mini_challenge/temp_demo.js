...
var mode=1;
let proj="com_sh_twoplayergame_mini_challenge"
let projP="com.sh.twoplayergame.mini.challenge.autojs"
let projR="com.sh.twoplayergame.mini.challenge"
function getPath(p){
    switch(mode){
        case 5:
...
function removeADs(x,y,w,h){
    click(528,1450)
}
function applyHarderAnPlay(x,y,w,h){
    gesture(1000,[536,850],[873,856])
    sleep(1000)
    click(540,1213)
}
...
function guid(){
    grant()
    tests[1].existApply(agree)
    tests[5].existApply(removeADs)
    // zutils.afollow([
...
//对于游戏我们只需要找到一个死循环即可
function singleLoop(){
    if(tests[2].existApply(ddclickCenter)){
        sleep(500)
        tests[3].existApply(ddclickCenter)
        sleep(500)
        tests[4].existApply(applyHarderAnPlay)
    }
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.sh.twoplayergame.mini.challenge",
