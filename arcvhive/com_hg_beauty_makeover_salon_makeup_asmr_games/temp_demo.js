...
var mode=1;
let proj="com_hg_beauty_makeover_salon_makeup_asmr_games"
let projP="com.hg.beauty.makeover.salon.makeup.asmr.games.autojs"
let projR="com.hg.beauty.makeover.salon.makeup.asmr.games"
function getPath(p){
    switch(mode){
        case 5:
...
function singleLoop(){
    if(!isGuid){
        while (tests[2].existApply(no_action)) {
            var t3 = tests[3];
            var t4 = tests[4];
            var t5 = tests[5]; 
            // 判断是否所有目标都成功 locate（即 x 坐标存在）
            var allLocated = t3 && t3.x != null && t4 && t4.x != null && t5 && t5.x != null; 
            if (t3) t3.locate();
            if (t4) t4.locate();
            if (t5) t5.locate(); 

            if(tests[2].existApply(no_action)){
            // 防止 gesture 参数出错，提前做保护
                if (t3.x != null && t3.y != null && t4.x != null && t4.y != null&& t5.x != null && t5.y != null) {
                    gesture(1200,
                        [t3.x, t3.y],
                        [t4.x, t4.y],
                        [t4.x + 20, t4.y],
        ...
        zutils.setColorFulSearch(true) //是否使用灰度
        if (tests[7].locate()) {   
            // while (tests[6].existApply(no_action)) {
                clog("执行 gesture...");
                gesture(2500,
                    [tests[7].x,tests[7].y],
                    [tests[7].x-40,TAP.y*0.4],
                    [tests[7].x-60,TAP.y*0.3],
                    [tests[7].x,tests[7].y],
                ) 
            // } 
        }
        zutils.setColorFulSearch(false) //是否使用灰度

        sleep(500)
        
        while (tests[9].existApply(no_action)) {
            var t9 = tests[9];
            var t10 = tests[10]; 
            t10.locate();
            t9.locate(); 
            clog("执行 gesture...");
            // 判断是否所有目标都成功 locate（即 x 坐标存在）
            var allLocated = t9 && t9.x != null && t10 && t10.x != null 

            // 防止 gesture 参数出错，提前做保护
            if(tests[8].existApply(no_action)){
                if (t9.x != null && t9.y != null && t10.x != null && t10.y != null) {
                    gesture(1000,
                        [t9.x, t9.y],
                        [t10.x, t10.y],
                        [t10.x + 20, t10.y], 
        ...
        if (tests[11].existApply(no_action)) {  
            TAP.clp(0.15,0.15)
            tests[12].waitMe(no_action,1000)
            while(tests[12].existApply(ddclickCenter)){ 
            }
        } 
    } 
}
