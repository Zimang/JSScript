...
var mode=1;
let proj="com_sh_draw_oneline_puzzle_game"
let projP="com.sh.draw.oneline.puzzle.game.autojs"
let projR="com.sh.draw.oneline.puzzle.game"
function getPath(p){
    switch(mode){
        case 5:
...
function gfuckU(x,y,w,h){
    gesture(3000,[290,462],[810,462],[810,1300],[290,1300],[290,462])
}
function shift(){ 
    // swipe(TAP.x,TAP.y,TAP.x,TAP.y-400,200) 
...
function logic3() { 
    tests[4].existApply(moreInfo)
    
}

//对于游戏我们只需要找到一个死循环即可
function singleLoop(){
    tests[3].existApply(gfuckU)
}
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.sh.draw.oneline.puzzle.game",
