...
var mode=1;
let proj="com_rofi_fakecall_prank_app"
let projP="com.rofi.fakecall.prank.app.autojs"
let projR="com.rofi.fakecall.prank.app"
function getPath(p){
    switch(mode){
        case 5:
...
// 定义三个无副作用的逻辑函数
function logic1() { 
    if(tests[8].existApply(ddclickCenter)){
        sleep(800)
...
function logic2() { 
    if(tests[1].existApply(ddclickCenter)){
        sleep(800)
        while(!tests[2].existApply((x,y,w,h)=>{
            click(63,65)
        })){
            sleep(1000)
            tests[9].existApply(()=>{
...
function logic3() { 
    if(tests[4].existApply(ddclickCenter)){
        sleep(800)
        while(!tests[5].existApply((x,y,w,h)=>{
            click(63,65)
        })){
            sleep(1000)
            tests[9].existApply(()=>{
