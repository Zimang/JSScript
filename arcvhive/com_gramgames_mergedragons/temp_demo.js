...
var mode=1;
let proj="com_gramgames_mergedragons"
let projP="com.gramgames.mergedragons.autojs"
let projR="com.gramgames.mergedragons"
function getPath(p){
    switch(mode){
        case 5:
...
function clickAcc(x,y,w,h){
    click(1152,736)
}
function click2Play(x,y,w,h){
    click(1586,960)
}
function clickBelow_217(x,y,w,h){
    click(x,y+217)
}
function clickScore(x,y,w,h){
    sleep(1000)
    click(x,y)
    sleep(1000)
}
function clickBook(x,y,w,h){
    click(907,663)
}
function guid(){
    grant()
    ...
    tests[1].existApply(clickAcc)
    if(tests[2].existApply(clickBelow_217)){
        sleep(500)
        tests[4].existApply(click2Play)
    }
    if(tests[4].existApply(no_action)){
        click(879,50) 
    }
    ...
    if(tests[7].existApply(clickBook)){
        while(!tests[5].existApply(clickScore)){
            if(tests[6].existApply(no_action)){
                break
            }
            gesture(1000,[704,543],[950,521]) 
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.gramgames.mergedragons",
