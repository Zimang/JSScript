...
var mode=1;
let proj="com_impostor_survival_run_challenge"
let projP="com.impostor.survival.run.challenge.autojs"
let projR="com.impostor.survival.run.challenge"
function getPath(p){
    switch(mode){
        case 5:
...
function below230(centerX,centerY,w,h){
    click(centerX,centerY+230)
    sleep(100) 
}
function above225(centerX,centerY,w,h){
    click(centerX,centerY-225)
    sleep(100) 
}
 
function spinBack(centerX,centerY,w,h){
    click(100,1300)
    sleep(100) 
}
function doublePrize(centerX,centerY,w,h){
    click(100,1300)
    sleep(100) 
}
...
function swipy(x,y,w,h){
    gesture(500,[x+w/2,y],[x-w/2,y])
    sleep(200)
    gesture(500,[x-w/2,y],[x+w/2,y])
}
...
function guid(){
    ...
    // ])
    while(tests[1].existApply(swipy)){
        click(99,82)
        tests[4].waitMe(ddclickCenter)
    }
    if(tests[9].existApply(ddclickCenter)){ 
        sleep(1000)
        while(!tests[1].existApply(swipy)){
            tests[10].existApply(swipy)
            tests[9].existApply(swipy)
            while(tests[1].existApply(swipy)){
                click(99,82)
...
//对于游戏我们只需要找到一个死循环即可
function singleLoop(){
    while(tests[5].existApply(no_action)){
        click(959,714)
        sleep(1000)
        tests[8].existApply(below230)
        tests[6].existApply(below230) 
    }

    tests[7].existApply(above225) 
    tests[6].existApply(spinBack) 
    tests[8].existApply(below230)
...
function main(){
    ...
    sleep(5000)
   // app.startActivity({ 
   //     packageName: "com.impostor.survival.run.challenge",
