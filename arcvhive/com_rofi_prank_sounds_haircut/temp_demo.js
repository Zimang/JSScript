...
var mode=1;
let proj="com_rofi_prank_sounds_haircut"
let projP="com.rofi.prank.sounds.haircut.autojs"
let projR="com.rofi.prank.sounds.haircut"
function getPath(p){
    switch(mode){
        case 5:
...
// 定义三个无副作用的逻辑函数
function logic1() { 
    if( horor.existApply(ddclickCenter)){
        sleep(500)
        while(!horor.existApply(no_action)){
            if(hororbk1.existApply(no_action)){
                clickBK()
                sleep(500)
            }
        }
    ...
    // meme.existApply(no_action)
    // animal.existApply(no_action)    
    
    // hororbk1.existApply(no_action)
    // memebk2.existApply(no_action)
    // animalbk3.existApply(no_action)
}

function logic2() { 
    if( meme.existApply(ddclickCenter)){
        sleep(500)
        while(!meme.existApply(no_action)){
            if(memebk2.existApply(no_action)){
                clickBK()
                sleep(500)
            }
        }
...
function logic3() { 
    if( animal.existApply(ddclickCenter)){
        sleep(500)
        while(!animal.existApply(no_action)){
            if(animalbk3.existApply(no_action)){
                clickBK()
                sleep(500)
            }
        }
...
let horor=new zutils.CachedBT(
    "cut_1_1.png",getPath(""),"buttom 1"
) 
let meme=new zutils.CachedBT(
    "cut_1_2.png",getPath(""),"buttom 1"
) 
let animal=new zutils.CachedBT(
    "cut_1_3.png",getPath(""),"buttom 1"
) 
let hororbk1=new zutils.CachedBT(
    "cut_2_1.png",getPath(""),"buttom 1"
) 
let memebk2=new zutils.CachedBT(
    "cut_3_1.png",getPath(""),"buttom 1"
) 
let animalbk3=new zutils.CachedBT(
