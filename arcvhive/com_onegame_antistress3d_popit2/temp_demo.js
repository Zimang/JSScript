...
var mode=1;
let proj="com_onegame_antistress3d_popit2"
let projP="com.onegame.antistress3d.popit2.autojs"
let projR="com.onegame.antistress3d.popit2"
function getPath(p){
    switch(mode){
        case 5:
...
function rs_back(x,y,w,h){ 
    click(935,304)
    sleep(200)
}
function rate_back(x,y,w,h){ 
    click(533,1530)
    sleep(200)
}
...
function logic2() { 
    ...
    sleep(1000)
    while(tests[7].existApply(no_action)){ 
        click(126,137)
    }
}
...
function logic3() { 
    ...
    sleep(1000)
    while(tests[8].existApply(no_action)){ 
        click(126,137)
    }
    sleep(1000)
...
//对于游戏我们只需要找到一个死循环即可
function singleLoop(){
    while(tests[4].existApply(rs_back)){}
    while(tests[5].existApply(rate_back)){}
    // requeseSSPermission()
}
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.onegame.antistress3d.popit2",
