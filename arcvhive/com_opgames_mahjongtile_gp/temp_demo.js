...
var mode=1;
let proj="com_opgames_mahjongtile_gp"
let projP="com.opgames.mahjongtile.gp.autojs"
let projR="com.opgames.mahjongtile.gp"
function getPath(p){
    switch(mode){
        case 5:
...
function singleLoop(){
    var count=0
    if(tests[16].existApply(no_action)){
        for(var i=3;i<13;i++){
            if(tests[i].existApply(ddclickCenter)){
                sleep(500)
                tests[i].existApply(ddclickCenter)
                sleep(500)
                tests[i].existApply(ddclickCenter)
                sleep(500)
                count++
        ...
        while(!tests[15].existApply(ddclickCenter)){
            click(216,150)
        }
    }
}
