...
var mode=1;
let proj="com_imangi_templerun2"
let projP="com.imangi.templerun2.autojs"
let projR="com.imangi.templerun2"
function getPath(p){
    switch(mode){
        case 5:
...
function clickOk(centerX,centerY,w,h){
    click(510,1243)
    sleep(100) 
}
...
function clickHead(x,y,w,h){
    click(93,1040)
    clog("YI HA")
    sleep(1000)
    if(tests[19].existApply(no_action)){
        clog("we are here")
        sleep(500)
        click(770,1270)
        click(770,1270)
        clog("clicked？")
        sleep(500)
        while(tests[17].existApply(clickOk)){}
        sleep(500)
...
function guid(){
    ...
    // ])
    while(tests[18].existApply(no_action)){
        click(746,832)
        sleep(500)
        click(551,1128)
    }
...
function singleLoop(){
    if(tests[4].existApply(ddclickCenter)){
        ...
        if(tests[6].existApply(ddclickCenter)){
            //向上
            for(var i=0;i<30;i++){
                gesture(1000,[TAP.x,TAP.y],[TAP.x,TAP.y-300])
            }
            //向右
            for(var i=0;i<30;i++){
                gesture(400,[TAP.x+200,TAP.y],[TAP.x,TAP.y])
            }
            //向左
            for(var i=0;i<30;i++){
                gesture(400,[TAP.x-200,TAP.y],[TAP.x,TAP.y])
            ...
            //向下
            for(var i=0;i<30;i++){
                gesture(400,[TAP.x,TAP.y],[TAP.x,TAP.y+300])
            }
            while(!tests[4].existApply(no_action)){
                click(1000,1700)
                sleep(500)
                click(160,1800)
                sleep(500)
