...
//http://s65q067944.zicp.fun/offerapp/com.tenmob.drawstickmanpuzzle.tar.gz
var mode=1;
let proj="com_tenmob_drawstickmanpuzzle"
let projP="com.tenmob.drawstickmanpuzzle.autojs"
let projR="com.tenmob.drawstickmanpuzzle"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    ...
    if(zutils.afollow([
        ["t","Trouble signing in?",0,0,backForGoogleProtect]
    ])){
        sleep(2000)
    }
    zutils.afollow([
...
function singleLoop(){
    if(tests[2].existApply(no_action)){
        gesture(1000,[TAP.x-200,TAP.y],[TAP.x+200,TAP.y])
    }
    
    tests[1].existApply(clickBelow) 
    sleep(2000)
}
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.tenmob.drawstickmanpuzzle",
