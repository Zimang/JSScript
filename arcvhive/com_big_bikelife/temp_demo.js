...
var mode=1;
let proj="com_big_bikelife"
let projP="com.big.bikelife.autojs"
let projR="com.big.bikelife"
function getPath(p){
    switch(mode){
        case 5:
...
function guid(){
    grant()    
    if( !tests[4].existApply(no_action)&&tests[1].existApply(ddclickCenter)){
        sleep(1200)
        if(tests[6].existApply(no_action)){
            ccclick(TAP.x*0.7,TAP.y,20,0)
            ccclick(TAP.x*1.3,TAP.y,20,0)
            ccclick(TAP.x*0.7,TAP.y,20,0)
            ccclick(TAP.x*1.3,TAP.y,20,0) 
            ccclick(TAP.x*0.7,TAP.y,20,0)  //教程1
            ccclick(TAP.x*1.3,TAP.y,20,0) 
...
function logic1() { 
    ...
    if(tests[4].existApply(no_action)&&tests[1].existApply(ddclickCenter)){
        tests[3].waitMe(ddclickCenter,500,1000)
    }
    sleep(200)
}
