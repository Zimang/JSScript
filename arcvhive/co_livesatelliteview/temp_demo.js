...
var mode=1;
let proj="co_livesatelliteview"
let projP="co.livesatelliteview.autojs"
let projR="co.livesatelliteview"
function getPath(p){
    switch(mode){
        case 5:
...
function right2Left(){ 
    ...
    gesture(1000,[TAP.x+200,TAP.y],[0,TAP.y])  
    sleep(500)
    return true
...
function guid(){
    grant()
    if(zutils.afollow([["t","All Languages",0,0] ])){
        while(!zutils.afollow([["t","English",0,500]])
            ||zutils.afollow([["t","Español",0,500,no_action]])
        ){
            gesture(500,[TAP.x,TAP.y],[TAP.x,TAP.y-200]) 
        }
        sleep(500)
        zutils.afollow([
            ["i","co.livesatelliteview:id/ic_done_white",0,0],
            right2Left,
            right2Left,
            right2Left,
            right2Left, 
            right2Left,  
            ["t","Use this version",0,500],
        ])
    }
    zutils.afollow([["t","ALLOW",0]])  
    zutils.afollow([
        ["i","co.livesatelliteview:id/iv_earth_3d",0,500,no_action],
        ["t","WHILE USING THE APP",500,500],
    ])  
...
function singleLoop(){
    zutils.afollow([
        ["i","co.livesatelliteview:id/btn_day_night",0,500,no_action],
        ["i","co.livesatelliteview:id/btn_zoom_in",0,1000,no_action],
        ["i","co.livesatelliteview:id/btn_zoom_in",0,500],
        ["i","co.livesatelliteview:id/btn_zoom_in",0,500],
        ["i","co.livesatelliteview:id/btn_zoom_in",0,500],
        ["i","co.livesatelliteview:id/btn_zoom_in",0,500],
        ["i","co.livesatelliteview:id/btn_zoom_in",0,500],
        ...
        ["i","co.livesatelliteview:id/iv_back",0,500],
    ])
    while(zutils.afollow([["i","co.livesatelliteview:id/txvAppName",0,500,no_action]])&&
    !tests[1].existApply(ddclickCenter)){
        gesture(1000,[TAP.x,TAP.y-200],[TAP.x,TAP.y-600]) 
    }
    zutils.afollow([ 
        ["t","WHILE USING THE APP",0,500],
    ])  
}


let der=zutils.createCircleDrawer(TAP.x,TAP.y,200,800) 
let c2p=zutils.create2PointGenerator(TAP.x,460,200,18,358)
function adapt(){ 
