...
var mode=1;
let proj="com_goodjobgames_matchvillains"
let projP="com.goodjobgames.matchvillains.autojs"
let projR="com.goodjobgames.matchvillains"
function getPath(p){
    switch(mode){
        case 5:
...
//对于游戏我们只需要找到一个死循环即可
function singleLoop(){
    tests[1].existApply(ff(()=>{
        gesture(150,[650,770],[645,896])
    }))
    while(!tests[3].existApply(ddclickCenter)){
        var dddd= c2p()
        if(i%2==1){
            gesture(getRandomInt(150,200),[dddd[0].x,dddd[0].y],[dddd[1].x,dddd[1].y]) 
        }else{
            gesture(getRandomInt(150,200),[dddd[1].x,dddd[1].y],[dddd[0].x,dddd[0].y])
        }  
...
let c2p=zutils.create2PointGenerator(980,1040,200,50,470)
function adapt(){ 
