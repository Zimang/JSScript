...
let test=new zutils.CachedBT(
 ...
 // 翻转 bitmap（原生 Android Bitmap 类型）
...
function testCapture(){
    ...
    try {
        images.saveImage(img, "/sdcard/1.jpg", "jpg");
        log("第一次 saveImage 成功");
    } catch (e) {
        log("第一次 saveImage 报错: " + e);
    }
    ...
    try {
        images.saveImage(img, "/sdcard/1_retry.jpg", "jpg"); // 🔥 再用一次 img
        log("img 被复用时 saveImage 成功");
    } catch (e) {
        log("img 被复用时报错: " + e);
    }
    ...
    try {
        ...
        log("第二张图 saveImage 成功");
    } catch (e) {
        log("第二张图 saveImage 报错: " + e);
    }
