...
var mode=1;
let proj="com_kayac_ScaleMan"
let projP="com.kayac.ScaleMan.autojs"
let projR="com.kayac.ScaleMan"
function getPath(p){
    switch(mode){
        case 5:
...
//对于游戏我们只需要找到一个死循环即可
function singleLoop(){
    tests[1].existApply(bigger)
    sleep(1000)
    tests[1].existApply(smaller)
}
...
function bigger(x,y,w,h){
    sleep(500)
    gesture(2000,[165,1605],[941,1605])
    sleep(500)
}

function smaller(x,y,w,h){
    sleep(500)
    gesture(2000,[941,1605],[165,1605])
    sleep(500)
}
...
function main(){
    ...
    app.launch(projR)
   // app.startActivity({ 
   //     packageName: "com.kayac.ScaleMan",
